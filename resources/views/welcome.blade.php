...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 my-10">
    <form action="/sonuclar/{{$examType->slug}}" method="POST">
        @csrf
        @if(empty($selected) && empty($exam_id))
            <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                <div class="sm:col-span-3">
                    <label for="student_id" class="block text-sm leading-5 font-medium text-gray-700">Öğrenci</label>
                    <select id="student_id" name="student_id"
                            class="mt-1 form-select block w-full pl-3 pr-10 py-2 text-base leading-6 border-gray-300 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5">
                        @foreach($students as $key=>$student)
                            @if(isset($selected))
                                <option value="{{$key}}" {{$key == $selected ? 'selected' : ''}}>{{$student}}</option>
                            @else
                ...
                <div class="sm:col-span-3">
                    <label for="exam_id" class="block text-sm leading-5 font-medium text-gray-700">Sınav</label>
                    <select id="exam_id" name="exam_id"
                            class="mt-1 form-select block w-full pl-3 pr-10 py-2 text-base leading-6 border-gray-300 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5">
                        <option value="">Tümünü Getir</option>
                        @foreach($exams as $key=>$exam)
                            @if(isset($exam_id))
                                <option value="{{$key}}" {{$key == $exam_id ? 'selected' : ''}}>{{$exam}}</option>
                            @else
        ...
        @else
            <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                ...
                <div class="sm:col-span-2">
                    <label for="lesson_id" class="block text-sm leading-5 font-medium text-gray-700">Ders</label>
                    <select id="lesson_id" name="lesson_id"
                            class="mt-1 form-select block w-full pl-3 pr-10 py-2 text-base leading-6 border-gray-300 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5">
                        <option value="">Tümünü getir</option>
                        @foreach($lessons as $key=>$lesson)
                            @if(isset($lesson_id))
                                <option value="{{$key}}" {{$key == $lesson_id ? 'selected' : ''}}>{{$lesson}}</option>
                            @else
    ...
    <div class="mt-4">
        @if(isset($selected) && isset($exam_id) && isset($labels))
            <canvas id="myChart"></canvas>
            <div class="legend"></div>
            <div class="flex justify-end mt-10">
            <span class="inline-flex rounded-md shadow-sm">
  <button type="button" onclick="PrintImage()"
          class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-gray-600 hover:bg-gray-500 focus:outline-none focus:border-gray-700 focus:shadow-outline-gray active:bg-gray-700 transition ease-in-out duration-150">
    <!-- Heroicon name: mail -->
<svg fill="currentColor" class="-ml-0.5 mr-2 h-4 w-4" viewBox="0 0 20 20">
  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @elseif(isset($selected) && (empty($exam_id) || isset($lesson_id)) && isset($labels))
            <canvas id="myChart"></canvas>
            <div class="legend"></div>
            <div class="flex justify-end mt-10">
            <span class="inline-flex rounded-md shadow-sm">
...
<svg fill="currentColor" class="-ml-0.5 mr-2 h-4 w-4" viewBox="0 0 20 20">
  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @elseif(empty($selected) && empty($exam_id))
            <h3>Lütfen öğrenci seçimi yapınız ve getir butonunu kullanınız.</h3>
        @else
            <h3>Böyle bir kayıt bulunmamaktadır!</h3>
        @endif
    </div>
    {{--    <livewire:graph />--}}
</div>
...
<script>
    ...
    var myChart = new Chart(ctx, {
        ...
        options: {
            title: {
                display: true,
                text: 'Deneme Bazlı Başarı Yüzdesi Grafiği'
            },
            // tooltips: {
            //     mode: 'index',
            //     intersect: false
    ...
    @endif
    function PrintImage() {
        printJS({printable: myChart.toBase64Image(), type: 'image', base64: true});
    }
</script>
@livewireScripts
</body>
